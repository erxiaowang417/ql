/** 
更新时间: 2021-09-1 09:03
Github Actions使用方法见[@lxk0301](https://raw.githubusercontent.com/lxk0301/scripts/master/githubAction.md) 使用方法大同小异

点击几篇文章和视频，自动获取阅读请求，在Github Actions中的Secrets新建name为'YOUTH_READ2'的一个值，拷贝抓包的请求体到下面Value的文本框中，添加的请求体越多，获得青豆次数越多，本脚本不包含任何推送通知
...
if (!$.isNode() && !YouthBody == true) {
    ...
    if ($.isNode()) {
        if (process.env.YOUTH_READ2 && process.env.YOUTH_READ2.indexOf('&') > -1) {
            YouthBodys = process.env.YOUTH_READ2.split('&');
            console.log(`您选择的是用"&"隔开\n`)
        } else if (process.env.YOUTH_READ2 && process.env.YOUTH_READ2.indexOf('\n') > -1) {
            YouthBodys = process.env.YOUTH_READ2.split('\n');
            console.log(`您选择的是用换行隔开\n`)
        } else {
            YouthBodys = [process.env.YOUTH_READ2]
        }
    } else if (!$.isNode() && YouthBody.indexOf("&") > -1) {
...
function bodyInfo() {
    return new Promise((resolve, reject) => {
        $.get(batHost('article/info/get.json?' + articlebody), async(err, resp, data) => {
        try {
          if (err) {
...
function AutoRead() {
    return new Promise((resolve, reject) => {
        $.post(batHost('article/complete.json', articlebody), async(err, response, data) => {
          try {
            if (err) {
              console.log(`${JSON.stringify(err)}`)
              console.log(`${$.name} API请求失败，请检查网路重试`)
              ...
              if (readres.items.complete == 1) {
                ...
                if (readres.error_code == '0' && data.indexOf("read_score") > -1 && readres.items.read_score > 0) {
                    ...
                    if ($.index % 2 == 0) {
                        if ($.isNode() && process.env.YOUTH_ATIME2) {
                            timebodyVal = process.env.YOUTH_ATIME2;
                        } else {
...
function readTime() {
    return new Promise((resolve, reject) => {
        $.post(batHost('user/stay.json', timebodyVal), async (err, resp, data) => {
          try {
            if (err) {
              console.log(`${JSON.stringify(err)}`)
              console.log(`${$.name} API请求失败，请检查网路重试`)
